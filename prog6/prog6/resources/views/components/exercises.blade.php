    ...
    @foreach($exer_list as $exer)
        <x-table.row>
            ...
            <x-table.body-row.cell>
                <a href="{{route('exercises.download',$exer['location'])}}" class="text-center text-blue-600">Tải
                    xuống</a>
            </x-table.body-row.cell>
            @if(Auth::user()->role==TEACHER)
                <x-table.body-row.cell>
                    <x-button onclick="showSubmitted('{{route('submitted.index',$exer['exer_id'])}}')">Xem</x-button>
                </x-table.body-row.cell>
                <x-table.body-row.cell>
                    <x-button
                        onclick="return cfDelExer('{{$exer['original_name']}}','{{$exer['exer_id']}}','exer_{{$exer['exer_id']}}')"
                        form="form_exer_{{$exer['exer_id']}}">
                        Xóa
                    </x-button>
                </x-table.body-row.cell>
            @else
